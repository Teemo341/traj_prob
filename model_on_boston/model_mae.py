...
class no_diffusion_model(nn.Module):
    ...
    def forward(self, condition: torch.Tensor, weighted_adj: torch.Tensor, y: Optional[torch.Tensor] = None,
        ...
        if condition is not None:
            ...
            # condition_emb = torch.zeros_like(tok_emb)
            condition_emb = 0
            for i in range(L):
                # add the condition to the embedding one by one
                condition_s = condition[:,:,i]  # (B, N)
                condition_s = condition_s.unsqueeze(-1).expand(B, N, L)
                condition_s_emb = self.token_embedding_table(condition_s.int())  # (B, N, L, C)
                condition_s_emb = torch.cat((tok_emb, condition_s_emb), dim=-1)  # (B, N, L, 2C)
                condition_s_score = torch.softmax(self.condition_proj_list[i](condition_s_emb), dim=-1)  # (B, N, L, 2)
                condition_s_emb = torch.einsum('bnld,bnldc->bnlc', condition_s_score, condition_s_emb.view(B, N, L, 2, -1))# (B, N, L, C)
