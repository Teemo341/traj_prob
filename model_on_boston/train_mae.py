...
parser.add_argument('--simulation_num', type=int, default=5000000)
...
parser.add_argument('--max_connection', type=int, default=3)
...
parser.add_argument('--observe_ratio', type=float, default=0.2)
...
loader = data_loader(city, data_dir, test_data_dir, True, simulation_num, test_simulation_num, block_size)
# loader.randomize_start_and_condition()
vocab_size = len(loader.pos) + 1
...
if load_dir_id:
    ...
    learning_rate *= lr_drop_rate**int(load_dir_id//save_iters)
...
for i in range (start+1, max_iters+1):
    model.train()
    # loader.randomize_start_and_condition()
    trajectory, adj_table, condition, special_mask = loader.load_train_batch(batch_size)
    ...
    logger_train_acc.append((torch.argmax(logits, dim=-1) == trajectory).float().mean().item())
    optimizer.step()
    lr_sched.step()

    if i % eval_iters == 0:
        model.eval()
        result_text = f'Iteration {i:<10}|  Loss: {loss.item():<10.8f}  |  Acc: {logger_train_acc[-1]:<7.2%}  |'
        trajectory, adj_table, condition, special_mask = loader.load_test_batch(test_size)
        trajectory = trajectory.to(device)
        if use_adj_table:
            adj = adj_table.to(device)
            # adj[:,:,:,1] = (adj[:,:,:,1]!=0).float() 
        ...
        for j in range(trajectory.shape[0]):
            for k in range(trajectory.shape[1]):
                if (torch.argmax(logits[j,k], dim = -1) == trajectory[j,k]).all():
                    real_true_rate += 1/(trajectory.shape[0]*trajectory.shape[1])
        logger_test_loss.append(loss.item())
        logger_test_acc.append(acc.item())
        logger_test_real_true_rate.append(real_true_rate)

        
        result_text += f'  Test Loss: {loss.item():<10.8f}  |  Test Acc: {acc:<7.2%}  |  Real True Rate: {real_true_rate:<7.2%}  |'
        print(result_text)

        del trajectory, adj_table, condition, special_mask, logits, loss
        # loader.randomize_start_and_condition()
    ...
    if i % save_iters == 0:
        ...
        print(f"Next learning rate: {lr_sched.get_last_lr()[0]}")
