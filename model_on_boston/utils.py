    ...
    for i in range(len(adj_table)):
        for j in range(len(adj_table[i])):
            if adj_table[i,j,1] != 0:
                weighted_adj_matrix[i,int(adj_table[i,j,0])] = adj_table[i,j,1]
...
def print_trajectory(predict_trajectory, test_trajectory, condition = None):
    for i in range(len(predict_trajectory)):
        ...
        for j in range(len(predict_trajectory[i])):
            result_txt_test = f"true {j+1:<3}: {np.array2string(test_trajectory[i][j])}"
            print(result_txt_test)
            
            result_txt_predict = f"pred {j+1:<3}: {np.array2string(predict_trajectory[i][j])}"
            print(result_txt_predict)

            if condition is not None:
                result_txt_condition = f"condition: {np.array2string(condition[i][j])}"
